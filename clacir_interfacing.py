...
def remove_nan(dataset):
    for i in range(0, len(dataset)):
        dataset[i][np.isinf(dataset[i])] = 0
        dataset[i][np.isnan(dataset[i])] = 0
...
def load_clacir_dataset(filepath):
    ...
    with open(os.path.join('dataset', 'clacir.pkl'), 'wb') as f:
        pickle.dump(clacir_dataset, f)
    return clacir_dataset


def generate_dataset(datastreams=None, panas_threshold=None):
    if datastreams is None:
        datastreams = [True, True, True, True, True]
    print("Collecting CLACIR dataset...")
    dataset_path = f'dataset/clacir.pkl'
    if os.path.exists(dataset_path):
        print("Pickled cLACIr dataset exists...\n")
        with open(dataset_path, 'rb') as f:
            dataset = pickle.load(f)
    else:
        dataset = load_clacir_dataset('dataset')
