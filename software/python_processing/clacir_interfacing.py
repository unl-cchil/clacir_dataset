...
def windowed_feature_extraction(window_size, write_pickle=True, datastreams=None, dataset_name="clasir"):
    ...
    if os.path.exists(dataset_path):
        ...
        for subject in range(0, len(data)):
            ...
            bvp_generator = split_set(data[subject][1], bvp_window_size, 64.0 / 4)
            eda_generator = split_set(data[subject][2], eda_window_size, 4.0 / 4)
            acc_generator = split_set(data[subject][0], acc_window_size, 32.0 / 4)
            temp_generator = split_set(data[subject][3], temp_window_size, 4.0 / 4)
            label_generator = split_set(labels[subject], label_window_size, 1)
