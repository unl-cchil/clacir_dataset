...
In Thayer & Stevens (2021), we conducted two experiments investigating the effects of human-animal interaction (HAI) on affect and cognition. In these experiments, participants experienced cognitive (working memory, attentional control) tasks before and after either a three-minute exposure to a dog (HAI condition) or a control task. Self-report measures of affect (mood, anxiety, stress) were collected repeatedly throughout the sessions. In addition to the affect and cognition measures, participants wore an Empatica E4 to record heart rate (via photoplethysmography), electrodermal activity (EDA), body temperature, and movement (via accelerometer). However, these data were not analyzed for the study, as the sampling rate was too low for robust measures of heart rate variability (Malik et al. 1996, Laborde et al. 2017).

Arce & Gehringer (2021) have investigated utilized the Biosensor Framework, a C# library to process and evaluate machine learning algorithms for time-series data from a wrist worn physiological sensor.  This library was validated against the Wearable Stress and Affect Detection (WESAD) dataset (Schmidt et al. 2018), which consists of 15 subjects wearing and ECG sensor, respiratory sensor, and the Empatica E4 during a stress induction task. The data come from a E4 output during baseline, control, and stress induction (using Trier Social Stress Test) phases. Arce & Gehringer (2021) applied machine learning algorithms to the E4 data to predict the participants' psychological state as indexed by the phase of the study.

Here, we plan to apply the machine learning analysis conducted by Arce & Gehringer (2021) to the Thayer & Stevens (2021) data. This data set has three phases/conditions: pre-intervention cognitive task, intervention (two conditions), post-intervention cognitive task. This generates five possible psychological states: pre-intervention cognitive load, active intervention (HAI condition), passive intervention (control condition), post-active intervention cognitive load, and post-passive intervention cognitive load. The overall design for the current study involves extracting features from the E4 data and using machine learning algorithms to predict the psychological state experience by the participant at the time of the physiological recording.
...
When the accelerometer features are included in the machine learning algorithm training, the movement associated with the active intervention make them the most important features for classification.  Excluding accelerometer features should improve the performance of the model and improve generalization. **Walker, why do you think that model performance will improve when accelerometer features are removed?**

4. In terms of physiological markers, electrodermal activity-based features will best predict psychological state.

Given an appropriately trained classifier, the higher order features from the electrodermal activity signal should be the best features to predict passive and active stress intervention exposures. **Walker, why do you make this prediction?**
...
The original Thayer and Stevens (2021) was a between-subjects design with two conditions. Participants were randomly assigned to the HAI intervention and control task. 

The current study will train four regression models, four multi-class models, and two binary models on the E4 data features to predict which part of the experimental session a participant was in.
...
**Walker, I don't think that there are any manipulated variables in this study. The manipulated variable in the original study was the active vs. passive intervention.**
...
**Walker, I would start by describing the E4 raw data here (EDA, PPG, ACC, TEMP).**
Raw data


Predictors
**Walker, we need to sort out how many states will be in our analyses. Will we only do these three or will we also do all five?**
We are classifying three different states: cognitive load, active intervention, and passive intervention.

Cognitive load is a state where the subject is performing cognitive tasks that require work to complete, thereby increasing the amount of attention and energy expended during the task (pre-intervention and post-intervention phases). 
...
**Walker, somewhere here we need to clarify what the rows/observations/instances are. That is, how do we collapse the continuous monitoring into a row?**

**Walker, since you have the performance measures in the analysis plan, I have removed them from here.**
...
**Walker, I think this is where you should describe how you created all of the features from the raw E4 data.**
...
**Walker, I think we need more details about training/testing sets and how they are fed into the models, as well as a description of the models.**

Performance metrics
Each multi-class model will be evaluated using their log loss, log loss reduction, macro accuracy, micro accuracy, and class precision, recall, and loss.

Each regression model will be evaluated using mean absolute error, mean squared error, root mean square error, and r-squared.

Each binary model will be evaluated using accuracy, area under the precision-recall curve, area under ROC curve, F1 score, negative precision and recall, and positive precision and recall.
...
**Walker, any transformations of the data, or do you use the raw feature data?**
...
**Walker, we need to think about this. My preference is to use Bayes factors to compare models.**
...
**Walker, did you have any outliers in the previous analysis?**
...
**Walker, have you checked for missing data?**
